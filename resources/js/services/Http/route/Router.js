...
export default class Router extends String {
    ...
    constructor(name, params, absolute = true, config) {
        ...
        this._config = config ?? routeConfig;
        this._config = {
            ...this._config,
            absolute
        };
        ...
        if (name) {
            if (!this._config.routes[name]) {
                throw new Error(
                    `Ziggy error: route '${name}' is not in the route list.`
                );
            }
    ...
    toString() {
        ...
        const unhandled = Object.keys(this._params)
            .filter(
                (key) =>
                    !this._route.parameterSegments.some(
                        ({name}) => name === key
                    )
            )
    ...
    current(name, params) {
        ...
        return (
            Object.entries(this._dehydrate(route))
    ...
    _parse(params = {}, route = this._route) {
        ...
        params = ["string", "number"].includes(typeof params)
            ? [params]
            : params;
        ...
        if (Array.isArray(params)) {
            ...
            (params.hasOwnProperty(Object.values(route.bindings)[0]) ||
    ...
    _dehydrate(route) {
        ...
        const dehydrate = (hydrated, template = "", delimiter) => {
            const [values, segments] = [hydrated, template].map((s) =>
                s.split(delimiter)
            );
